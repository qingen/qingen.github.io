...
14 年刚接触 $DNN$ 的时候推导的公式，现在把它放在博客里面，可以方便的给大家参考，目前还在整理中，可以先不用看，等格式整理好了后大家再看

![](/assets/images/2020/ai/xunfei02.jpg)

> 曾经的培训场景，也正是这位同学让我走进了语音识别的方向，同样也正是因为他，让我离开了讯飞，深深的记得当时离开前喊我谈话的时候，问我去哪了，我说去阿里了，他说 去做测试 吗。。。
...
-   $\text{output}$层函数$\text{softmax}$：$z_{m} = g\left( \text{net}_{m} \right) = \frac{\exp\left( \text{net}_{m} - \operatorname{}\text{net}_{i} \right)}{\sum_{k = 1}^{c}{\exp\left\lbrack \text{net}_{k} - \operatorname{}\text{net}_{i} \right\rbrack}}$ ，其中$\text{net}_{m}$表示$\text{output}$层第$m$个节点的输入，$z_{m}$表示$\text{output}$层第$m$个节点的输出。

根据输入层信号计算出$hidden1$层的输入，$hidden1$层的输出，然后再计算出$hidden2$层的输入和输出，以此类推推出所有隐层的输入和输出。最后我们再根据$hidden6$层的输出计算出$\text{output}$层的输入和输出。具体过程如下：
...
$J = \frac{1}{2}\sum_{m = 1}^{c}{(t_m - z_m)^{2}}$ 公式(1)

其中，待定参数是各层的权值$w_{}^{(k)}$，以及偏置$b^{(k)}$。根据梯度下降法，我们可以令：

$\Delta w^{(k)} = - \eta\frac{\partial J}{\partial w^{(k)}}$                                                                公式(2)

$\Delta b^{(k)} = - \eta\frac{\partial J}{\partial b^{(k)}}$                                                                 公式(3)

其中$\eta$是学习率。可以通过公式2和公式3分别计算出每一层权值和偏置的增量，从而实现对权值、偏置的更新。下面我们分别推导出各层$w_{}^{(k)}$和$b^{(k)}$的增量，$k = 1,2,...,7$，$s$表示每一层的下标。

我们做如下定义：

上误差：$e_{s}^{k \uparrow} = \frac{\partial J}{\partial y_{s}^{k}}$ 公式(4)

下误差：$e_{s}^{k \downarrow} = \frac{\partial J}{\partial\text{ne}t_{s}^{k}}$ 公式(5)

又因为

$y_{s}^{k} = f(net_{s}^{k})$             公式(6)
...
其中$k = 2,3,...,7$，$s,r$分别表示$k$层和$k - 1$层的下标，且$s,r \in \lbrack 1,\operatorname{}\rbrack$*。*注意对于$\text{hidden}1$层，其权值增量由下误差$e_{j}^{(1) \downarrow}$和输入$x_{i}$决定，偏置增量仅与下误差$e_{j}^{(1) \downarrow}$有关。即：

$\Delta w_{\text{ji}}^{(1)} = - \eta e_{j}^{(1) \downarrow} \cdot x_{i}$     公式(10)

$\Delta b_{j}^{(1)} = - \eta e_{j}^{(1) \downarrow}$ 公式(11)

到此我们已经知道了各层权值增量，偏置增量和下误差的关系。下面我们就该计算每一层的下误差的大小。首先我们推导下输出层的下误差$e_{m}^{(7) \downarrow}$的大小，过程如下：
$$
e_{m}^{(7) \uparrow} = \frac{\partial J}{\partial z_{m}} = - (\operatorname{} - \operatorname{})
$$
由公式7可知：
$$
e_{m}^{(7) \downarrow} = \frac{\partial J}{\partial\text{ne}t_{m}^{(7)}} = e_{m}^{(7) \uparrow} \cdot g'(\operatorname{})
$$
继续计算$hidden6$的误差如下：
$$
...
$$
e_{l}^{(6) \downarrow} = \frac{\partial J}{\partial\text{ne}t_{l}^{(6)}} = e_{l}^{(6) \uparrow} \cdot f'(\operatorname{})
$$
同理我们可以推出各层的上下误差，即：
$$
...
$$
e_{s}^{(\operatorname{k}) \downarrow} = \frac{\partial J}{\partial\text{ne}t_{s}^{(\operatorname{k})}} = e_{s}^{(\operatorname{k}) \uparrow} \cdot f'(\operatorname{})
$$ 公式(13)

其中，$k = 1,2,...,6$，s表示当$k$前层的下标，r表示$k + 1$层的下标。

公式12和公式13可以看出，每层的上误差都和上一层的下误差以及他们这两层之间的权值w有关，而每一层的下误差则又和本身的上误差有关。因此只要计算出最后一层（输出层）的上误差，我们就可以分别推导出各层的上下误差。这个过程是从输出层往输入层的迭代推导，即误差的反向传播。图2我们给出误差反传的整个过程。
...
$w_{\text{sr}}^{(k)} = w_{\text{sr}}^{(k)} + \Delta w_{\text{sr}}^{(k)}$ 公式(16)

$b_{s}^{(k)} = b_{s}^{(k)} + \Delta b_{s}^{(k)}$ 公式(17)

最后我们将前向计算和反向传播的全过程重现一遍，见图3。

图2. 误差反向传播

图3. DNN前向计算、反向误差传播及更新
...
1.  从内存中读取N个样本$\{\operatorname{},\operatorname{},...,\operatorname{}\}$，每个样本d维，共d\*N个浮点数。将这些数从内存复制到GPU内存中。在实际任务中一般假设N=1024,d=440，每个float的取值范围是（-100，100）。

2.  计算$\text{hidden}1$层的输入，$\text{ne}t_{k,j}^{\left( 1 \right){\sum_{i = 1}^{d}{x_{k,i}w_{\text{ji}}^{\left( 1 \right)}}}_{j}^{(1)}}$，$k = 1,2,...,N$表示样本的标号。用矩阵表示为：$\text{NE}T^{\left( 1 \right)} = XW^{\left( 1 \right)} + B^{\left( 1 \right)}$，其中$\text{NE}T^{\left( 1 \right)}$为$N*\operatorname{}{}$ 的矩阵$\text{NE}T^{(1)} = \begin{bmatrix}
    \text{ne}t_{1,1}^{(1)} & \text{ne}t_{1,2}^{(1)} & \text{...} & \text{ne}t_{1,\operatorname{}}^{(1)} \\
    \text{ne}t_{2,1}^{(1)} & \text{ne}t_{2,2}^{(1)} & \text{...} & \text{ne}t_{2,\operatorname{}}^{(1)} \\
    \text{...} & \text{...} & \text{...} & \text{...} \\
    \text{ne}t_{\operatorname{N,}1}^{(1)} & \text{ne}t_{\operatorname{N,}2}^{(1)} & \text{...} & \text{ne}t_{\operatorname{}}^{(1)} \\
    \end{bmatrix}$，$X$为$N*d$的矩阵$X = \left\lbrack \begin{matrix}
    x_{1,1} & x_{1,2} & \text{...} & x_{1,d} \\
    x_{2,1} & x_{2,2} & \text{...} & x_{2,d} \\
    \text{...} & \text{...} & \text{...} & \text{...} \\
    x_{N,1} & x_{N,2} & \text{...} & x_{N,d} \\
    \end{matrix}\lbrack\rbrack \right\rbrack$，$W^{\left( 1 \right)}$为$d*\operatorname{}{}$的矩阵$W^{(1)} = \begin{bmatrix}
    w_{1,1}^{(1)} & w_{1,2}^{(1)} & \text{...} & w_{1,\operatorname{}}^{(1)} \\
    w_{2,1}^{(1)} & w_{2,2}^{(1)} & \text{...} & w_{2,\operatorname{}}^{(1)} \\
    \text{...} & \text{...} & \text{...} & \text{...} \\
    w_{d,1}^{(1)} & w_{d,2}^{(1)} & \text{...} & w_{d,\operatorname{}}^{(1)} \\
    \end{bmatrix}$，$B^{\left( 1 \right)}$也是$N*\operatorname{}{}$的矩阵$B^{(1)} = \begin{bmatrix}
    b_{1,1}^{(1)} & b_{1,2}^{(1)} & \text{...} & b_{1,\operatorname{}}^{(1)} \\
    b_{2,1}^{(1)} & b_{2,2}^{(1)} & \text{...} & b_{2,\operatorname{}}^{(1)} \\
    \text{...} & \text{...} & \text{...} & \text{...} \\
    b_{\operatorname{N,}1}^{(1)} & b_{\operatorname{N,}2}^{(1)} & \text{...} & b_{\operatorname{}}^{(1)} \\
    \end{bmatrix}$。

3.  计算$\text{hidden}1$层的输出，$y_{k,j}^{(1)} = f(net_{k,j}^{\left( 1 \right)})$。 $Y^{(1)} = \begin{bmatrix}
    f(\operatorname{}) & f(net_{1,2}^{(1)}) & \text{...} & f(net_{1,\operatorname{}}^{(1)}) \\
    f(net_{2,1}^{(1)}) & f(net_{2,2}^{(1)}) & \text{...} & f(net_{2,\operatorname{}}^{(1)}) \\
    \text{...} & \text{...} & \text{...} & \text{...} \\
    f(net_{\operatorname{N,}1}^{(1)}) & f(\operatorname{}) & \text{...} & f(net_{\operatorname{}}^{(1)}) \\
    \end{bmatrix}$。

4.  计算$\text{hidden}2$层的输入值。$\text{NE}T^{\left( 2 \right)} = Y^{(1)}W^{\left( 2 \right)} + B^{\left( 2 \right)}$，其中$\text{NE}T^{\left( 2 \right)}$为$N*\operatorname{}{}$ 的矩阵，$Y^{\left( 1 \right)}$为$N*\operatorname{}{}$ 的矩阵，$W^{\left( 2 \right)}$为$\operatorname{}{}$的矩阵，$B^{\left( 2 \right)}$也是$N*\operatorname{}{}$的矩阵。

5.  计算$\text{hidden}2$层的输出值。$y_{k,j}^{(2)_{k,j}^{\left( 2 \right)}}$。重复步骤三和步骤四直到计算出所有层的输入和输出值（总共6个隐层）。

6.  计算$\text{output}$层的输入值，$Z = Y^{(6)}W^{\left( 7 \right)} + B^{\left( 7 \right)}$，其中$Z$为$N*c$ 的矩阵，$Y^{\left( 6 \right)}$为$N*\operatorname{}{}$ 的矩阵，$W^{\left( 7 \right)}$为$\operatorname{}{}$的矩阵，$B^{\left( 7 \right)}$也是$N*c$的矩阵。

7.  计算$\text{output}$层的输出值$z_{m} = g(\operatorname{})$。

8.  计算$\text{output}$层的上误差。$e_{k,m}^{(7) \uparrow} = - (t_{k,m}\mathrm{-}\mathrm{z}_{k,m})$，并由此计算$\text{output}$层的下误差$e_{k,m}^{(7) \downarrow} = e_{k,m}^{(7) \uparrow} \cdot g'(\operatorname{})$。

9.  计算$\text{hidden}6$层的上误差$e_{k,l}^{(6) \uparrow} = e_{k,m}^{(7) \downarrow} \cdot w_{\text{ml}}^{(7)}$，即$E_{}^{(6) \uparrow} = E_{}^{(7) \downarrow} \cdot W^{(7)T}$。

10. 计算$\text{hidden}6$层的下误差$e_{k,l}^{(6) \downarrow} = e_{k,l}^{(6) \uparrow \operatorname{})}$。

11. 重复步骤9、步骤10计算其余各隐层的上下误差。直到计算到$\text{hidden}1$层为止。

12. 计算权值和偏置值的增量。$\Delta w_{\text{ji}}^{(\operatorname{s})} = - \eta\sum_{k = 1}^{N\sum}e_{k,j}^{(\operatorname{s}) \downarrow_{i}^{(\operatorname{s} - 1)}}$，$\Delta b_{j}^{(\operatorname{s})} = - \eta\sum_{k = 1}^{N\sum}e_{k,j}^{(\operatorname{s}) \downarrow}s$表示层数$s = 2,3,...,7$ ，$j$ 表示$s$层的下标，$i$表示$s - 1$层的下标。对于$\text{hidden}1$层有：$\Delta w_{\text{ji}}^{(1)} = - \eta\sum_{k = 1}^{N}{e_{k,j}^{(1) \downarrow} \cdot x_{i}}$，$\Delta b_{j}^{(1)} = - \eta\sum_{k = 1}^{N}e_{k,j}^{(1) \downarrow}$，$j$ 表示$\text{hidden}1$层的下标，$i$表示$\text{input}$层的下标。

13. 矫正更新量，，其中$k = 1,2,...,7$，表示上一次更新时的$\Delta w_{\text{ji}}^{(\operatorname{k})}$值，表示上一次更新时的$\Delta b_{j}^{(k)}$值。

14. 更新权重和偏置值。$\left\{ \begin{matrix}
    w_{\text{ji}}^{(k)} = w_{\text{ji}}^{(k)} + \Delta w_{\text{ji}}^{(k)} \\
    b_{j}^{(k)} = b_{j}^{(k)} + \Delta b_{j}^{(k)} \\
    \end{matrix} \right.\ $其中$k = 1,2,...,7$。
