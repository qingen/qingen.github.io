---
layout: post
title: softmax regression 理论部分
category: pytorch
tag: pytorch
excerpt: 回顾回顾基本概念也不错
mathjax: true
keywords: softmax regression  回归
---

## 写在前面
晚上陪儿子睡觉前，在床上用3个枕头搭建了个小房子，儿子在里面躺着玩，很开心啊，但是妈妈生气了，说都晚上10点钟了，还不睡觉，正是长个子的好时刻，还不睡觉，以后成小矮子了。。
...
下图采用神经网络图描绘了上式的计算。softmax回归同线性回归一样，也是一个单层神经网络。由于每个输出$o_1, o_2, o_3$的计算都要依赖于所有的输入$x_1, x_2, x_3, x_4$，softmax回归的输出层也是一个全连接层。

![img](/assets/images/pytorch/softmax-regression.jpg)
...
$$
\boldsymbol{W} = \begin{bmatrix} w_{11} & w_{12} & w_{13} \\
w_{21} & w_{22} & w_{23} \\
w_{31} & w_{32} & w_{33} \\
w_{41} & w_{42} & w_{43} \\
\end{bmatrix}, \quad \boldsymbol{b} = \begin{bmatrix} b_1 & b_2 & b_3 \end{bmatrix},
$$
...
$$
\begin{aligned}
\boldsymbol{O} &= \boldsymbol{X} \boldsymbol{W} + \boldsymbol{b},\\ \boldsymbol{\hat{Y}} &= \text{softmax}(\boldsymbol{O}), 
\end{aligned}
$$
